...
int jpeg_read_bit() {   // read bit
    if(curbit == 0) {
        ...
        if(curbyte == 0xFF) {
            ...
            if(curbyte >= 0xD0 && curbyte <= 0xD7) memset(jdc,0,sizeof(int) * 3);
            if(curbyte == 0) curbyte = 0xFF; else jpeg_read_byte();
        }
    }
    int i = (curbyte >> (7 - curbit++)) & 0x01;
...
void jpeg_idct(float *idata) {   // aa&n algorithm inverse DCT
                ...
                for(V = 0; V < component_info[plane].v; V++)
                    for(H = 0; H < component_info[plane].h; H++) {
                        ...
                        for(y = 0, i = 0; y < 8; y++)
                            for(x = 0; x < 8; x++, i++) {
                                j = jpeg_zigzag[i];
                                dct[i] = vector[j] * qtable[k][j] * aanscale[x] * aanscale[y];
                            }
                        jpeg_idct(dct);
                        for(y = 0; y < 8; y++)
                            for(x = 0; x < 8; x++) {
                                ...
                                if(scaleh[plane] == 1 && scalev[plane] == 1) {
                                    i = X + x + (H << 3);
                                    j = Y + y + (V << 3);
                                    if(i < width && j < height) data[((j * width + i) << 2) + plane] = c;
                                }
...
bool load_jpeg(const char *name){
    ...
    memset(component_info,0,sizeof(component_info)*3);//really owerflow??
...
//    if(!data) return 0;
	...
	CON.write(0,"jpg %i components",num_components);
    //---------------------   flip?
   /* unsigned char *ddata=new unsigned char[width*height*4];//crush at multithread load after few iters
    if(ddata){
